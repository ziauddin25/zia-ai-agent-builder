...
import type { AgentData, SavedAgent, ToastMessage } from '../types';

interface SavedAgentsProps {
  data: AgentData | null;
  canvasRef: React.RefObject<{ loadAgent: (agent: SavedAgent) => void } | null>;
  onCountChange?: (count: number) => void;
}

export function SavedAgents({ data, canvasRef, onCountChange }: SavedAgentsProps) {
  ...
  const cardGrid = (
    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3 lg:grid-cols-3 xl:grid-cols-4">
      {agents.map((agent) => (
        <div
          key={agent.id}
          className="group rounded-xl border border-slate-800 bg-slate-900 p-3 transition-all duration-200 hover:border-slate-700 hover:shadow-lg hover:shadow-violet-500/5 sm:p-4"
        >
    ...
    return (
      <div>
        ...
        <div className="flex items-center justify-between border-t border-slate-800 bg-slate-900 px-3 py-3 lg:px-6 lg:py-4 ">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-3 text-xs font-medium text-cyan-400 transition-all duration-300 hover:bg-cyan-400/20 cursor-pointer"
            >
        ...
        {expanded && (
          <div className="max-h-[35vh] overflow-y-auto border-t border-slate-800 bg-slate-950 px-3 py-3 lg:px-6 lg:py-4">
            {cardGrid}
          </div>
        )}
      </div>
