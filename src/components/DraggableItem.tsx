...
export default function DraggableItem({ id, name, subtitle, type, onSelect }: DraggableItemProps) {
  ...
  if (isMobile) {
    return (
      <button
        onClick={onSelect}
        className={cn(
          'group flex w-full cursor-pointer items-center gap-3 rounded-lg border p-2.5 text-left transition-all duration-150',
          config.border,
          `${config.bg} ${config.hover} active:scale-95`,
        )}
      >
  ...
  return (
    <div
      ...
      {...attributes}
      className={cn(
        'group flex cursor-grab items-center gap-3 rounded-lg border p-2.5 transition-all duration-150 active:cursor-grabbing',
        config.border,
        isDragging
